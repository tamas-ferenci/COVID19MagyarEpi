...
resGr <- grData(predData(RawData, "Poisson", 95)$m, params$reportSImu, params$reportSIsd)
resGrSw <- grSwData(RawData, lapply( 1:(nrow(RawData)-7+1), function(i) predData(RawData[i:(i+7-1)], "Poisson", 95)$m ),
...
```{r}
epicurvePlot(predData(RawData, "Poisson", params$reportConf)$pred, TRUE, TRUE, TRUE, TRUE, params$reportConf)
```

`r grText(predData(RawData, "Poisson", params$reportConf)$m)`
...
```{r}
epicurvePlot(predData(RawData, "Poisson", params$reportConf, NA, 7)$pred, TRUE, TRUE, FALSE, TRUE, NA)
```

Számszerű előrejelzések a következő hétre:

```{r}
res <- round_dt(predData(RawData, "Poisson", params$reportConf, NA, 7)$pred)
res$Pred <-  paste0(res$fit, " (", res$lwr, "-", res$upr, ")")
knitr::kable(res[, .(`Dátum` = Date, `Esetszám [fő]` = CaseNumber, `Becsült esetszám (95%-os CI) [fő]` = Pred)])
```
